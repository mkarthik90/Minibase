...
public class BMDriver extends TestDriver implements GlobalConst {
	...
	protected boolean test1() {
		...
		for (int i = 0; i < 3; i++) {
			...
			System.out.print("\n  Test 1 does simple test on buffer using the"
					+ replacealgo[i] + "algorithm");
			...
			try {
				SystemDefs.JavabaseBM
						.unpinPage(firstPid, false /* not dirty */);
			} catch (Exception e) {
			...
			for (pid.pid = firstPid.pid, lastPid.pid = pid.pid + numPages; status == OK
				...
				try {
					SystemDefs.JavabaseBM.pinPage(pid, pg, /* emptyPage: */
							true);
				} catch (Exception e) {
					status = FAIL;
				...
				if (status == OK) {
					...
					if (status == OK) {
						if (data != (pid.pid) + 99999) {
							status = FAIL;
							System.err
									.print("*** Read wrong data back from page "
											+ pid.pid + "\n");
						}
					}
					...
					if (status == OK) {
						try {
							...
							status = FAIL;
							System.err.print("*** Could not unpin page "
									+ pid.pid + "\n");
							e.printStackTrace();
						}
					}
			...
			for (pid.pid = firstPid.pid; pid.pid < lastPid.pid; pid.pid = pid.pid + 1) {
				...
				try {
					...
					status = FAIL;
					System.err
							.print("*** Error freeing page " + pid.pid + "\n");
					e.printStackTrace();
				}
	...
	protected boolean test2() {
		...
		for (int i = 0; i < 3; i++) {
			...
			System.out
					.print("  - Try to pin more pages than there are frames\n");
			try {
				firstPid = SystemDefs.JavabaseBM.newPage(pg, numPages);
			} catch (Exception e) {
				System.err.print("*** Could not allocate " + numPages);
			...
			for (pid.pid = firstPid.pid + 1, lastPid.pid = firstPid.pid
					+ numPages - 1; status == OK && pid.pid < lastPid.pid; pid.pid = pid.pid + 1) {
			...
			if (status == OK
					&& SystemDefs.JavabaseBM.getNumUnpinnedBuffers() != 0) {
				status = FAIL;
			...
			if (status == OK) {
				...
				if (status == OK) {
					...
					if (status == OK) {
						status = FAIL;
						System.err
								.print("The expected exception was not thrown\n");
					} else {
						status = OK;
					}
				}
	...
	protected boolean test3() {
		...
		for (int i = 0; i < 3; i++) {
					...
					if (status == OK) {
						if (pid.pid % 20 != 12) {
							try {
								...
								status = FAIL;
								System.err
										.print("*** Could not unpin dirty page "
												+ pid.pid + "\n");
							}
						}
					}
			...
			for (i = 0; i < pids.length; i++) {
				try {
					SystemDefs.JavabaseBM.freePage(pids[i]);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
